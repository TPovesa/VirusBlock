        ...
        messages: [
            {
                ...
                content: [
                    ...
                    'Если файл похож на библиотеку, framework, toolkit, shared lib, host-plugin или dev SDK, не делай вывод по одиночным risky API. В таких файлах допустимы installer/bootstrap, share helpers, plugin management, UI helpers и сетевые обвязки без злого умысла.',
                    'Для library/framework-кода важна именно цельная цепочка злоупотребления: скрытый автозапуск, кража данных, удалённая догрузка и выполнение, stealth-поведение, закрепление или явное вредоносное действие. Простое наличие helper-методов, импортов, Intent/FileProvider, requests/httpx, zip/shutil, PluginsController, DexClassLoader, socket/server API или device-helper слоёв недостаточно.',
                    ...
                    'Для библиотек и shared framework-кода block допустим только когда явно видна цельная вредоносная цепочка. Сам по себе bootstrap, installer, plugin management, share/export, network helper, storage helper или dev tooling не повод для block.',
                    'Если файл похож на MandreLib-подобную общую библиотеку для плагинов, оценивай его как framework surface: отдельно отмечай спорные подсистемы, но не объявляй весь файл вредоносным без явного злоупотребления.',

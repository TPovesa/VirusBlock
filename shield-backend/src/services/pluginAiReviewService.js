...
function rewriteBenignToolingBullet(bullet) {
    ...
    if (/удал[её]н|exec|выполн|оболочк|reverse shell|код из сети/.test(lowered)) {
        return 'Есть механизмы загрузки или подключения модулей. Для такого типа файлов это может быть штатной частью архитектуры, но важно проверить их источник и сценарий использования.';
    }
    if (/утечк|send|post|upload|token|session|cookie|секрет|данн/.test(lowered)) {
        return 'Есть сетевой обмен данными. Стоит отдельно проверить, какие именно данные уходят наружу и соответствует ли это назначению модуля.';
    }
    if (/file|файл|remove|delete|unlink|rmtree|zip|shutil|import модулей|импорт/.test(lowered)) {
        return 'Есть операции с файлами и импортом модулей. Для библиотек это бывает штатной частью работы, но их лучше оценивать в контексте сценария использования.';
    }
    return value;
}
...
function buildBenignToolingSummary(input, verdictSuggestion) {
    ...
    if (verdictSuggestion === 'clean') {
        return `${prefix}файл больше похож на библиотеку или служебный модуль. Внутри есть системные и сетевые helper-механизмы, но по текущим данным они выглядят как часть обычной инфраструктуры плагина, а не как вредоносный сценарий.`;
    }
    return `${prefix}файл больше похож на библиотеку или служебный модуль. Внутри есть служебные системные и сетевые подсистемы, поэтому его лучше оценивать в контексте назначения, но явная вредоносная цепочка по текущим данным не подтверждена.`;
}

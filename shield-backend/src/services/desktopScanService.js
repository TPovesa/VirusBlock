...
async function getReleaseManifest() {
    const [rows] = await pool.query(
        `SELECT platform, channel, version, sha256, download_url, install_command, file_name, notes_json, metadata_json, updated_at
         FROM release_artifacts
         WHERE is_latest = 1
         ORDER BY FIELD(platform, 'android', 'windows', 'linux', 'shell', 'site'), updated_at DESC`
    ).catch(() => [[]]);
        ...
        artifacts: [
            {
                ...
                sha256: '',
                download_url: `${baseRaw}/android/neuralv-android-release.apk`,
                install_command: '',
                file_name: 'neuralv-android-release.apk',
                notes: ['Android APK публикуется через GitHub Actions release-builds branch'],
                metadata: {}
            },
            {
                platform: 'windows',
                ...
                version: 'rolling-main',
                sha256: '',
                download_url: `${baseRaw}/windows/neuralv-windows.zip`,
                install_command: '',
                file_name: 'neuralv-windows.zip',
                notes: ['Compose Desktop Windows build'],
                metadata: {}
            },
            {
                platform: 'linux',
                ...
                version: 'rolling-main',
                sha256: '',
                download_url: `${baseRaw}/linux/neuralv-linux.tar.gz`,
                install_command: '',
                file_name: 'neuralv-linux.tar.gz',
                notes: ['Compose Desktop Linux build'],
                metadata: { daemonUrl: `${baseRaw}/shell/neuralvd-linux.tar.gz` }
            },
            {
                platform: 'shell',
                channel: 'beta',
                version: 'rolling-main',
                sha256: '',
                download_url: `${baseRaw}/shell/neuralv-shell-linux.tar.gz`,
                install_command: 'curl -fsSL https://sosiskibot.ru/neuralv/install/linux.sh | bash',
                file_name: 'neuralv-shell-linux.tar.gz',
                notes: ['Bubble Tea shell client with systemd installer'],
                metadata: { daemonUrl: `${baseRaw}/shell/neuralvd-linux.tar.gz` }
            },
            {
                platform: 'site',
                ...
                version: 'rolling-main',
                sha256: '',
                download_url: `${baseRaw}/site/neuralv-site.zip`,
                install_command: '',
                file_name: 'neuralv-site.zip',
                notes: ['Static /neuralv site bundle'],
                metadata: {}
            }
        ]
    };
